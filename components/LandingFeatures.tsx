...
function HybridRefinedSection({ number, title, desc, icon, badge, visual, reversed }: any) {
  return (
    <div className={`flex flex-col md:flex-row items-center gap-20 md:gap-40 ${reversed ? 'md:flex-row-reverse' : ''}`}>
       ...
       <motion.div 
         ...
         viewport={{ once: true }}
         transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
         className="flex-1 w-full bg-white/[0.01] border border-white/5 rounded-[3rem] aspect-video flex items-center justify-center p-12 overflow-hidden relative group"
       >
    ...
    <div className="relative w-full h-full flex items-center justify-center p-8">
       <div className="relative w-full max-w-[320px] flex flex-col gap-3">
          {[0, 1, 2].map((i) => (
             ...
             >
                <div className={`w-2 h-2 rounded-full ${i === 0 ? 'bg-red-400' : i === 1 ? 'bg-blue-400' : 'bg-green-400'}`} />
                <div className="flex-1 space-y-2">
                   <div className="w-24 h-2 bg-white/10 rounded-full" />
                   <div className="w-12 h-1.5 bg-white/5 rounded-full" />
                </div>
                <div className="flex gap-2">
                   <div className="w-8 h-4 rounded-md bg-white/5 border border-white/5" />
                </div>
             </motion.div>
          ))}
    ...
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
       {[0, 1, 2].map((i) => (
         <motion.div
           ...
           className="absolute w-64 h-44 bg-white/5 border border-white/10 rounded-[2.5rem] flex flex-col p-8 space-y-4 shadow-2xl backdrop-blur-sm"
           style={{ 
              zIndex: 3 - i,
              translateY: `${i * -28}px`,
              scale: 1 - i * 0.1,
         ...
         >
            <div className="flex justify-between items-center">
               <FileText className="w-6 h-6 text-purple-400/50" />
